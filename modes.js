(function(){
'use strict';
const MODES={
 bp:{name:'BP',open:18,summary:'隨機開 18 角；流程為紅1禁、藍1禁、紅1選、藍1選；紅2禁、藍2禁、紅2選、藍2選；藍3禁、紅3禁、藍3選、紅3選。角色固定進入對應座位。'},
 tavern:{name:'酒館模式',open:4,summary:'開場隨機開 4 角；紅禁、藍禁、紅選、藍選，每次操作後補入一名新角色，重複三輪。座次與選角順序皆為紅藍紅藍紅藍。'},
 fearless:{name:'無懼',open:52,summary:'52 角全開；各方前幾局使用過的角色僅該方不能再選。紅、藍各先禁一角，選角順序紅藍紅藍藍紅，可各使用一次 Insert Ban。'},
 cm01:{name:'CM01',open:20,summary:'隨機開 20 角；紅方禁一角、藍方禁兩角。選角順序紅藍紅藍藍紅，可自由安排座位並各使用一次 Insert Ban。'},
 rdbp:{name:'RD+BP',open:20,summary:'52 角隨機各分 26 角；雙方各移除 3 角後，從剩餘 46 角隨機開 20 角，接著依 CM01 流程禁選。'},
 re:{name:'Re.星杯戰爭',open:52,summary:'固定為無懼選角五連戰、每方總思考 200 秒，耗盡後每手 10 秒讀秒、人機對戰。每場將遭遇一支曾參賽隊伍。'}
};
const PICK_ORDER=['red','blue','red','blue','blue','red'];
const SEAT_ORDER=[['red',0],['blue',0],['red',1],['blue',1],['blue',2],['red',2]];
const BP_STEPS=[
 ['bp-ban','red',0],['bp-ban','blue',0],['bp-pick','red',0],['bp-pick','blue',0],
 ['bp-ban','red',1],['bp-ban','blue',1],['bp-pick','red',1],['bp-pick','blue',1],
 ['bp-ban','blue',2],['bp-ban','red',2],['bp-pick','blue',2],['bp-pick','red',2]
];
const TAVERN_STEPS=[
 ['tavern-ban','red',0],['tavern-ban','blue',0],['tavern-pick','red',0],['tavern-pick','blue',0],
 ['tavern-ban','red',1],['tavern-ban','blue',1],['tavern-pick','red',1],['tavern-pick','blue',1],
 ['tavern-ban','red',2],['tavern-ban','blue',2],['tavern-pick','red',2],['tavern-pick','blue',2]
];
const ROLE_KEYWORDS={
 '封印師':['劍意末路','探索旅','獸耳同盟'],'記錄者':['繼承者'],'結界師':['神子創臨','獸耳同盟'],'靈魂術士':['學院都市','探索旅'],'冒險家':['探索旅'],'魔劍士':['劍意末路','魔裝少女'],'魔槍':['女武神計畫','魔裝少女'],'魔弓':['御神之亂','弓神候補','魔裝少女'],'瘟疫法師':['黑衣聖教','神霾之下','幕後策首'],'異教徒':['歧路姊妹'],'陰陽師':['巫女暴走','獸耳同盟'],
 '暗殺者':['特經社','獵人'],'風之劍聖':['劍意末路','八想流','繼承者'],'格鬥家':['圓桌統合','繼承者'],'劍帝':['圓桌統合','繼承者'],'精靈射手':['御神之亂','精靈同盟'],'獵巫人':['紅蓮叛亂','獵人'],'女僕長':['神子創臨','特經社','獵物'],'神箭手':['御神之亂','弓神候補'],'獸靈武士':['巫女暴走','巫女機關','獸耳同盟'],
 '傳教士':['歧路姊妹'],'紅衣主教':['神子創臨','精靈解放','幕後策首'],'女武神':['女武神計畫','聖廷武裝'],'神官':['圓桌統合'],'聖殿騎士':['紅蓮叛亂','神之子','聖廷武裝'],'聖弓':['黑衣聖教','聖廷武裝'],'聖女':['黑衣聖教','神之子'],'聖槍騎士':['懲戒部隊','聖廷武裝'],'聖庭檢察士':['紅蓮叛亂','懲戒部隊'],'守護天使':['女武神計畫','幕後策首'],'月之女神':['神霾之下'],
 '蒼炎魔女':['賢者之書','學院都市'],'紅蓮騎士':['紅蓮叛亂','神之子'],'狂戰士':['精靈解放','角鬥場'],'染污者':['神子創臨','青梅竹馬','角鬥場'],'噬神者':['神子創臨','精靈同盟','角鬥場'],'血色劍靈':['圓桌統合'],'血之巫女':['巫女暴走','巫女機關'],'勇者':['賢者之書','青梅竹馬'],'仲裁者':['探索旅'],
 '蝶舞者':['巫女暴走','學術交流'],'矜貴之女':['神子創臨'],'靈符師':['巫女暴走','學院都市','獸耳同盟','學術交流'],'魔法少女':['精靈同盟'],'祈禱師':['巫女暴走','學術交流'],'神秘學者':['精靈解放','精靈同盟','單相思'],'賢者':['賢者之書','學院都市','繼承者'],'星墜女巫':['紅蓮叛亂','學院都市','獵物'],'英靈人形':['女武神計畫'],'元素師':['精靈解放','精靈同盟','單相思'],'戰鬥法師':['紅蓮叛亂','學院都市']
};
const KEYWORD_RULES={
 '八想流':{effect:'對手分數 > 89 時，對手 -10'},'弓神候補':{effect:'分數 +3',add:3},'歧路姊妹':{effect:'最終分數固定為 88',fixed:88},'神之子':{effect:'分數 +2',add:2},'巫女機關':{effect:'分數 +2',add:2},'青梅竹馬':{effect:'分數 +5',add:5},'角鬥場':{effect:'對手分數 -5',opp:-5},'特經社':{effect:'最終分數至少 85',min:85},'獵人&獵物':{effect:'分數 -10',add:-10,special:'hunter'},'聖廷武裝':{effect:'分數 +5',add:5},'獸耳同盟':{effect:'分數 +5',add:5},'精靈同盟':{effect:'分數 +5',add:5},'懲戒部隊':{effect:'分數 +3',add:3},'學院都市':{effect:'分數 +3',add:3},'神霾之下':{effect:'分數 -5',add:-5},'探索旅':{effect:'分數 +5',add:5},'幕後策首':{effect:'對手分數 -5',opp:-5},'繼承者':{effect:'分數 +5',add:5},'單相思':{effect:'分數 +3',add:3},'學術交流':{effect:'分數 +5',add:5},'魔裝少女':{effect:'分數 +5',add:5},'劍意末路':{effect:'分數 -5',add:-5},'神子創臨':{effect:'分數 +5',add:5},'御神之亂':{effect:'分數 -5',add:-5},'女武神計畫':{effect:'分數 +5',add:5,special:'valkyrie'},'黑衣聖教':{effect:'分數 +3',add:3},'圓桌統合':{effect:'分數 +5',add:5},'紅蓮叛亂':{effect:'有紅蓮騎士 +5；否則 -5',special:'redlotus'},'巫女暴走':{effect:'有血之巫女 +5；否則 -5',special:'bloodwitch'},'賢者之書':{effect:'有賢者 +5；否則 -5',special:'sage'},'精靈解放':{effect:'分數 +5',add:5}
};
const KEYWORD_ORDER=Object.keys(KEYWORD_RULES);
let state=null,timerId=null,storyTimer=null,learningEnabled=false,seatDockCleanup=null;
let rePreviewObjectUrl='';
const TEST_BUILD=true; // 正式上線版必須改為 false，建議組合不會出現在正式版。
const q=id=>document.getElementById(id), teamName=t=>t==='red'?'紅方':'藍方', other=t=>t==='red'?'blue':'red';
const esc=value=>String(value??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));
const shuffle=a=>{a=a.slice();for(let i=a.length-1;i;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]]}return a};
const byId=id=>characters.find(c=>c.id===Number(id));
document.querySelectorAll('.versus-mode-btn').forEach(btn=>btn.onclick=()=>selectMode(btn.dataset.mode));
q('versusSeriesLength').onchange=()=>state&&render();q('versusTimer').onchange=()=>state&&startTimer();
q('versusLearningToggle').onclick=()=>{learningEnabled=!learningEnabled;const b=q('versusLearningToggle');b.classList.toggle('active',learningEnabled);b.setAttribute('aria-pressed',String(learningEnabled));b.textContent=learningEnabled?'◇ 學習模式：開啟':'◇ 學習模式：關閉'};
q('profileStartBtn').onclick=()=>startSeries();
function selectMode(mode){
 gameMode=mode;document.querySelectorAll('.mode-btn').forEach(b=>b.classList.remove('active'));document.querySelector(`[data-mode="${mode}"]`)?.classList.add('active');
 q('chainConfig').classList.add('hidden');q('versusConfig').classList.toggle('hidden',mode==='re');q('reConfig').classList.toggle('hidden',mode!=='re');
 if(mode!=='re')q('versusRuleSummary').textContent=MODES[mode].summary;q('profileError').textContent='';
}
function teamKey(value){return String(value||'').normalize('NFKC').toLocaleLowerCase().replace(/[^\p{L}\p{N}]/gu,'')}
function reTeamForSide(side){return side===state?.playerTeam?state.teamName:(state?.reOpponent?.name||'對戰隊伍')}
function sideLabel(side){return state?.mode==='re'?`${teamName(side)}・${reTeamForSide(side)}`:teamName(side)}
function reOpponentAvatar(){
 const team=state?.reOpponent?.sourceName||state?.reOpponent?.name,rows=(window.RE_DIALOGUES||[]).filter(x=>x.team&&teamKey(x.team)===teamKey(team)),ids=shuffle([...new Set(rows.map(x=>byName(x.character)?.id).filter(Boolean))]);
 return ids[0]||state?.reOpponent?.rounds?.flat().map(byName).find(Boolean)?.id||characters[0]?.id;
}
function chooseReTeamRecords(){
 const groups=new Map();for(const rec of window.RE_TEAM_DRAFTS||[]){if(!isValidReTeamRecord(rec)){console.warn('已排除異常隊伍資料：',rec?.name);continue}const key=teamKey(rec.name);if(!groups.has(key))groups.set(key,[]);groups.get(key).push(rec)}
 const teams=[];for(const list of groups.values()){list.slice(0,2).forEach((rec,index)=>teams.push({...rec,name:index?`${rec.name}支隊`:rec.name,sourceName:rec.name}))}
 return shuffle(teams).slice(0,5);
}
function isValidReTeamRecord(record){
 if(!record||!Array.isArray(record.rounds)||record.rounds.length!==5)return false;
 const used=new Set();
 for(const round of record.rounds){
   if(!Array.isArray(round)||round.length!==3||new Set(round).size!==3)return false;
   for(const name of round){if(!byName(name)||used.has(name))return false;used.add(name)}
 }
 return true;
}
function recordCompleteness(record){return Array.from({length:5},(_,index)=>Math.min(3,Array.isArray(record?.rounds?.[index])?record.rounds[index].filter(name=>byName(name)).length:0)).reduce((a,b)=>a+b,0)}
function completeReTeamRecord(record,displayName=record?.name||'參賽隊伍'){const rounds=[],used=new Set();for(let index=0;index<5;index++){const valid=[...new Set((Array.isArray(record?.rounds?.[index])?record.rounds[index]:[]).filter(name=>byName(name)))].slice(0,3);const available=shuffle(characters.map(c=>c.name).filter(name=>!used.has(name)&&!valid.includes(name)));while(valid.length<3&&available.length)valid.push(available.pop());valid.forEach(name=>used.add(name));rounds.push(valid)}return{...record,name:displayName,sourceName:record?.name||displayName,rounds}}
function startSeries(){
 const entered=q('playerNameInput').value.trim().slice(0,30),isRe=gameMode==='re';
 if(isRe&&(!entered||!favoriteName)){q('profileError').textContent='請先輸入隊伍名稱並選擇本命角色';return}
 playerName=entered||'玩家';
 const total=isRe?5:(Number(q('versusSeriesLength').value)||1);
 state={mode:gameMode,total,match:0,matchActive:false,history:[],used:{red:new Set(),blue:new Set()},phase:null,pool:[],banned:new Set(),seats:{red:[null,null,null],blue:[null,null,null]},insert:{red:true,blue:true},log:[],rdHands:null,rdRemoved:{red:new Set(),blue:new Set()},seconds:0,delegated:false,playMode:isRe?'ai':q('versusBattleType').value,learning:isRe?false:learningEnabled,playerTeam:null,marks:{diamond:new Set(),danger:new Set()},suggestions:{active:false,high:new Set(),low:new Set(),highScore:null,lowScore:null,team:null},teamName:playerName,favorite:isRe?favoriteName:'',reTeams:isRe?chooseReTeamRecords():[],rePlayerUsed:new Set(),reFavoriteUsed:false,reFavoriteSeen:0,reOpponent:null,reMainTime:{red:200,blue:200},reByoyomi:false,reClockTeam:null,tavernSeen:[]};
 q('profileError').textContent='';q('profilePanel').classList.add('hidden');q('gamePanel').classList.add('hidden');q('resultPanel').classList.add('hidden');q('chainResultPanel').classList.add('hidden');q('versusPanel').classList.remove('hidden');
 if(isRe)showReStory();else startMatchSafe();
}
function showReStory(){
 clearTimeout(storyTimer);const team=state.teamName.endsWith('隊')?state.teamName:`${state.teamName}隊`;
 const lines=[`如果當時選${state.favorite}就好了...`,'....','......','種種的回憶仍然困著你，難以走出過往失利的陰霾。','你走在路上，急促的喇叭聲越來越近，車燈在你眼前瞬間越放越大...','....','一道白光之後，你睜開眼',`『${team}，請趕快入座』，你被主辦的聲音喚回神，`,'旁邊的隊友也催促你趕緊入座。','『原來...我穿越回那場比賽開始前嗎？』'];
 const p=q('versusPanel');p.innerHTML=`<div class="re-story-page"><button id="reStorySkip" class="re-story-skip">Skip</button><div id="reStoryBox" class="re-story-box" role="button" tabindex="0"><div id="reStoryText" class="re-story-text"></div><button id="reStoryEnter" class="vs-btn re-story-enter hidden">確認，進入比賽</button></div></div>`;
 let line=0,pos=0,done=false,entering=false;const text=q('reStoryText'),box=q('reStoryBox');
 const draw=()=>{text.innerHTML=lines.map((x,i)=>`<div class="${i===line&&!done?'typing':''}">${esc(i<line||done?x:i===line?x.slice(0,pos):'')}</div>`).join('')};
 const tick=()=>{if(done)return;if(pos<lines[line].length){pos++;draw();storyTimer=setTimeout(tick,46);return}if(line<lines.length-1){line++;pos=0;draw();storyTimer=setTimeout(tick,320);return}done=true;draw();q('reStoryEnter').classList.remove('hidden')};
 const speed=()=>{if(done)return;pos=lines[line].length;draw();clearTimeout(storyTimer);storyTimer=setTimeout(tick,120)};
 const enter=event=>{event?.preventDefault();event?.stopPropagation();if(entering)return;entering=true;clearTimeout(storyTimer);state.matchActive=false;p.innerHTML='';startMatchSafe()};
 box.onclick=e=>{if(e.target.id==='reStoryEnter')enter(e);else speed()};box.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();speed()}};q('reStorySkip').addEventListener('click',enter,{once:true});draw();tick();
}
function startMatchSafe(){for(let attempt=0;attempt<3;attempt++){try{startMatch();return true}catch(error){console.error(`第 ${state?.history?.length+1||1} 場載入失敗（第 ${attempt+1} 次）：`,error);clearInterval(timerId);state.matchActive=false;if(state?.mode==='re'&&state.reTeams){const replacement=chooseReTeamRecords()[0];if(replacement)state.reTeams[state.history.length]=replacement}}}const p=q('versusPanel');if(p)p.innerHTML=`<div class="vs-result"><div class="vs-title">本場資料載入失敗</div><p>系統已排除異常隊伍，請按下方按鈕重新抽選對手。</p><button id="vsRecover" class="vs-btn">重新載入本場</button><button id="vsHome" class="vs-btn">回到首頁</button></div>`;if(q('vsRecover'))q('vsRecover').onclick=startMatchSafe;if(q('vsHome'))q('vsHome').onclick=quit;return false}
function startMatch(){
 if(state.matchActive)return;if(state.history.length>=state.total){if(state.mode==='re')renderReFinal();return}state.matchActive=true;state.match=state.history.length+1;state.banned=new Set();state.seats={red:[null,null,null],blue:[null,null,null]};state.insert={red:true,blue:true};state.log=[];state.delegated=false;state.suggestions={active:false,high:new Set(),low:new Set(),highScore:null,lowScore:null,team:null};state.playerTeam=state.playMode==='ai'?(Math.random()<.5?'red':'blue'):null;
 if(state.mode==='re'){
   state.reOpponent=state.reTeams[state.match-1]||chooseReTeamRecords()[0];
   const cpu=other(state.playerTeam),prior=(state.reOpponent?.rounds||[]).slice(0,state.match-1).flat().map(name=>characters.find(c=>c.name===name)?.id).filter(Boolean);
   state.used={red:new Set(),blue:new Set()};state.used[state.playerTeam]=new Set(state.rePlayerUsed);state.used[cpu]=new Set(prior);state.reOpponentAvatarId=reOpponentAvatar();
   if(!state.rePlayerUsed.has(byName(state.favorite)?.id))state.reFavoriteSeen++;
 }
 if(state.mode==='tavern'){
   state.pool=shuffle(characters.map(c=>c.id)).slice(0,4);state.tavernSeen=[...state.pool];setTavernStep(0);
 }else if(state.mode==='rdbp'){
   const all=shuffle(characters.map(c=>c.id));state.rdHands={red:new Set(all.slice(0,26)),blue:new Set(all.slice(26))};state.rdRemoved={red:new Set(),blue:new Set()};state.pool=all;state.phase={kind:'rd-handover',team:'red',controller:'red'};
 }else{
   state.pool=['fearless','re'].includes(state.mode)?characters.map(c=>c.id):shuffle(characters.map(c=>c.id)).slice(0,MODES[state.mode].open);beginBan();
 }
 log(`第 ${state.match} 場開始：${MODES[state.mode].name}${state.mode==='re'?'・對手 '+state.reOpponent.name:''}`);render();if(state.phase.kind!=='rd-handover')startTimer();
}
function beginBan(){
 if(state.mode==='tavern')setTavernStep(0);
 else if(state.mode==='bp')setBpStep(0);
 else state.phase={kind:'ban',step:0,team:'red',controller:'red'};
}
function setBpStep(step){const item=BP_STEPS[step];if(!item)return finishMatch();const [kind,team,seat]=item;state.phase={kind,bpStep:step,team,controller:team,seat}}
function setTavernStep(step){const item=TAVERN_STEPS[step];if(!item)return finishMatch();const [kind,team,seat]=item;state.phase={kind,tavernStep:step,team,controller:team,seat}}
function tavernReveal(){const occupied=new Set([...state.pool,...state.banned,...state.seats.red.filter(Boolean),...state.seats.blue.filter(Boolean)]),left=characters.map(c=>c.id).filter(id=>!occupied.has(id));if(!left.length)return null;const id=left[Math.floor(Math.random()*left.length)];state.pool.push(id);state.tavernSeen.push(id);log(`酒館迎來新角色：${byId(id).name}`);return id}
function advanceTavernStep(){const next=state.phase.tavernStep+1;if(next>=TAVERN_STEPS.length)return finishMatch();setTavernStep(next);render();startTimer()}
function phaseText(){const p=state.phase;if(!p)return'';
 if(p.kind==='rd-handover')return`請將裝置交給${sideLabel(p.controller)}`;
 if(p.kind==='rd-remove')return`${sideLabel(p.controller)}從自己的 26 角移除角色（剩 ${p.remaining} 角）`;
 if(p.kind==='tavern-ban')return`${sideLabel(p.team)}${p.seat+1}禁角，完成後酒館補入一名新角色`;
 if(p.kind==='tavern-pick')return`${sideLabel(p.team)}${p.seat+1}選角，完成後酒館補入一名新角色`+(state.delegated?`（由${sideLabel(p.controller)}代選）`:'');
 if(p.kind==='bp-ban')return`${p.seat===0?sideLabel(p.team)+(p.team==='red'?'1':'1'):sideLabel(p.team)+(p.seat+1)}玩家禁角`;
 if(p.kind==='bp-pick')return`${sideLabel(p.team)}${p.seat+1}玩家選角，將固定放入${teamName(p.team)}${p.seat+1}`+(state.delegated?`（由${sideLabel(p.controller)}代選）`:'');
 if(p.kind==='ban')return`${sideLabel(p.controller)}禁角`;
 if(p.kind==='insert')return`${sideLabel(p.controller)}使用 Insert Ban`;
 if(p.kind==='pick')return`${sideLabel(p.team)}選角`+(state.delegated?`（由${sideLabel(p.controller)}代選）`:'');return''}
function availableFor(id){const p=state.phase;if(!p||state.banned.has(id)||state.seats.red.includes(id)||state.seats.blue.includes(id))return false;
 if(p.kind==='rd-remove')return state.rdHands[p.controller].has(id)&&!state.rdRemoved[p.controller].has(id);
 if(['fearless','re'].includes(state.mode)&&['ban','insert'].includes(p.kind)&&state.used[other(p.controller)].has(id))return false;
 if(p.kind==='pick'||p.kind==='bp-pick'){if(['fearless','re'].includes(state.mode)&&state.used[p.team].has(id))return false;}
 return state.pool.includes(id);
}
function choose(id){const p=state.phase;
 if(p?.kind==='pick'&&state.pendingPick===id){state.pendingPick=null;render();startTimer();return;}
 if(!availableFor(id))return;
 if(p.kind==='tavern-ban'){state.banned.add(id);state.pool=state.pool.filter(x=>x!==id);log(`${teamName(p.controller)}禁用 ${byId(id).name}`);tavernReveal();advanceTavernStep();return;}
 if(p.kind==='tavern-pick'){state.seats[p.team][p.seat]=id;state.pool=state.pool.filter(x=>x!==id);log(`${teamName(p.controller)}為${teamName(p.team)}${p.seat+1}選擇 ${byId(id).name}`);state.delegated=false;tavernReveal();advanceTavernStep();return;}
 if(p.kind==='rd-remove'){state.rdRemoved[p.controller].add(id);log(`${teamName(p.controller)}移除 ${byId(id).name}`);p.remaining--;if(!p.remaining){if(p.controller==='red'){clearInterval(timerId);state.log=[];state.phase={kind:'rd-handover',team:'blue',controller:'blue'};render();return}else{const left=characters.map(c=>c.id).filter(x=>!state.rdRemoved.red.has(x)&&!state.rdRemoved.blue.has(x));state.pool=shuffle(left).slice(0,20);state.log=[];beginBan();}}render();startTimer();return;}
 if(['ban','bp-ban','insert'].includes(p.kind)){state.banned.add(id);log(`${teamName(p.controller)}禁用 ${byId(id).name}${p.kind==='insert'?'（Insert Ban）':''}`);advanceAfterBan();return;}
 if(p.kind==='bp-pick'){state.seats[p.team][p.seat]=id;log(`${teamName(p.controller)}為${teamName(p.team)}${p.seat+1}選擇 ${byId(id).name}`);state.delegated=false;advanceBpStep();return;}
 if(p.kind==='pick'){state.pendingPick=id;if(state.suggestions)state.suggestions.active=false;renderSeatsChoice();}
}
function advanceAfterBan(){const p=state.phase;
 if(p.kind==='bp-ban'){advanceBpStep();return;}
 else if(p.kind==='insert'){state.insert[p.controller]=false;state.phase=p.resume;}
 else if(p.step===0)state.phase={kind:'ban',step:1,team:'blue',controller:'blue'};
 else if(p.step===1&&!['fearless','re'].includes(state.mode))state.phase={kind:'ban',step:2,team:'blue',controller:'blue'};
 else beginPicks();render();startTimer();
}
function skipBan(){if(!state.phase||!['ban','bp-ban','insert'].includes(state.phase.kind))return;log(`${teamName(state.phase.controller)}空過禁角`);advanceAfterBan();}
function advanceBpStep(){const next=state.phase.bpStep+1;if(next>=BP_STEPS.length)return finishMatch();setBpStep(next);render();startTimer();}
function beginPicks(){state.phase={kind:'pick',index:0,team:PICK_ORDER[0],controller:PICK_ORDER[0]};render();startTimer();}
function renderSeatsChoice(){clearInterval(timerId);const p=state.phase,id=state.pendingPick;const free=state.seats[p.team].map((x,i)=>x==null?i:-1).filter(i=>i>=0);if(free.length===1)return placePick(free[0]);render(free);}
function placePick(seat){const p=state.phase,id=state.pendingPick;if(id==null||state.seats[p.team][seat]!=null)return;state.seats[p.team][seat]=id;log(`${teamName(p.controller)}為${teamName(p.team)}${seat+1}選擇 ${byId(id).name}`);state.pendingPick=null;state.delegated=false;const next=p.index+1;if(next>=PICK_ORDER.length)return finishMatch();state.phase={kind:'pick',index:next,team:PICK_ORDER[next],controller:PICK_ORDER[next]};render();startTimer();}
function useInsert(){const p=state.phase;if(!p||p.kind!=='pick'||state.pendingPick!=null)return;const blocker=other(p.team);if(!state.insert[blocker])return;state.phase={kind:'insert',team:blocker,controller:blocker,resume:{...p}};render();startTimer();}
function onTimeout(){const p=state.phase;if(!p)return;if(p.kind==='tavern-ban'){const id=state.pool.find(availableFor);if(id)choose(id);return;}if(p.kind==='tavern-pick'){state.delegated=true;p.controller=other(p.team);log(`${teamName(p.team)}選角超時，改由${teamName(p.controller)}代選`);render();startTimer();return;}if(state.pendingPick!=null&&p.kind==='pick'){placePick(bestSeatFor(p.team,state.pendingPick));return}if(['ban','bp-ban','insert'].includes(p.kind)){skipBan();return;}if(p.kind==='rd-remove'){const id=state.pool.find(availableFor);if(id)choose(id);return;}if(['pick','bp-pick'].includes(p.kind)){state.delegated=true;p.controller=other(p.team);log(`${teamName(p.team)}選角超時，改由${teamName(p.controller)}代選`);render();startTimer();}}
function startTimer(){clearInterval(timerId);if(state?.mode==='re'){const team=state.phase?.controller||state.phase?.team||'red';state.reClockTeam=team;state.reByoyomi=(state.reMainTime?.[team]||0)<=0;state.seconds=state.reByoyomi?10:state.reMainTime[team];renderTimer();timerId=setInterval(()=>{state.seconds--;if(!state.reByoyomi)state.reMainTime[team]=Math.max(0,state.seconds);if(state.seconds<=0&&!state.reByoyomi){state.reMainTime[team]=0;state.reByoyomi=true;state.seconds=10;renderTimer();return}renderTimer();if(state.seconds<=0){clearInterval(timerId);onTimeout()}},1000);return}const limit=Number(q('versusTimer').value)||0;state.seconds=limit;if(!limit){renderTimer();return}renderTimer();timerId=setInterval(()=>{state.seconds--;renderTimer();if(state.seconds<=0){clearInterval(timerId);onTimeout()}},1000)}
function renderTimer(){const e=q('vsTimer');if(!e)return;e.textContent=state?.mode==='re'?(state.reByoyomi?`讀秒 ${state.seconds}s`:`總 ${state.seconds}s`):(state.seconds?`${state.seconds}s`:'∞');e.classList.toggle('urgent',state.seconds>0&&state.seconds<=5)}
function reDialogueFor(team,char,fallback){
 const rows=(window.RE_DIALOGUES||[]).filter(x=>x.character===char),same=rows.filter(x=>x.team&&teamKey(x.team)===teamKey(team)),generic=rows.filter(x=>!x.team),pool=same.length?same:generic.length?generic:rows;
 return pool.length?pool[Math.floor(Math.random()*pool.length)].text:fallback;
}
function keywordHue(name){const index=Math.max(0,KEYWORD_ORDER.indexOf(name));return(index*137.508)%360}
function keywordBadgeStyle(name){const hue=keywordHue(name);return`--kw-bg:hsl(${hue} 48% 28%);--kw-border:hsl(${hue} 78% 67%);--kw-text:hsl(${hue} 95% 90%)`}
function keywordBadges(events=[],showEffect=true){if(!events.length)return'';return`<div class="re-keywords">${events.map(event=>`<span class="re-keyword" style="${keywordBadgeStyle(event.name)}"><b>${esc(event.name)}</b>${showEffect?`<small>${esc(event.applied)}</small>`:''}</span>`).join('')}</div>`}
function lineupKeywordEvents(ids,opponentRaw){
 const names=ids.map(id=>byId(id)?.name).filter(Boolean),counts=new Map();for(const name of names)for(const keyword of ROLE_KEYWORDS[name]||[])counts.set(keyword,(counts.get(keyword)||0)+1);
 const active=[];for(const name of KEYWORD_ORDER){const rule=KEYWORD_RULES[name];let triggered=(counts.get(name)||0)>=2;if(rule.special==='hunter')triggered=names.some(role=>(ROLE_KEYWORDS[role]||[]).includes('獵人'))&&names.some(role=>(ROLE_KEYWORDS[role]||[]).includes('獵物'));if(rule.special==='valkyrie')triggered=names.includes('女武神')&&(counts.get(name)||0)>=2;if(!triggered)continue;
  let own=Number(rule.add||0),opp=Number(rule.opp||0),applied=rule.effect;if(name==='八想流'){opp=opponentRaw>89?-10:0;applied=opp?'對手分數 -10':'對手未超過 89，未扣分'}if(rule.special==='redlotus'){own=names.includes('紅蓮騎士')?5:-5;applied=`分數 ${own>0?'+':''}${own}`}if(rule.special==='bloodwitch'){own=names.includes('血之巫女')?5:-5;applied=`分數 ${own>0?'+':''}${own}`}if(rule.special==='sage'){own=names.includes('賢者')?5:-5;applied=`分數 ${own>0?'+':''}${own}`}
  active.push({name,applied,own,opp,min:rule.min??null,fixed:rule.fixed??null});
 }
 return active;
}
function applyKeywordEffects(redIds,blueIds,redRaw,blueRaw){const events={red:lineupKeywordEvents(redIds,Number(blueRaw)),blue:lineupKeywordEvents(blueIds,Number(redRaw))},scores={red:Number(redRaw),blue:Number(blueRaw)};for(const side of ['red','blue'])for(const event of events[side]){scores[side]+=event.own;scores[other(side)]+=event.opp}for(const side of ['red','blue']){for(const event of events[side])if(event.min!=null)scores[side]=Math.max(scores[side],event.min);for(const event of events[side])if(event.fixed!=null)scores[side]=event.fixed}return{red:scores.red,blue:scores.blue,events}}
function finishMatch(){
 if(!state.matchActive||state.history.length>=state.total)return;state.matchActive=false;clearInterval(timerId);state.phase=null;for(const t of ['red','blue'])state.seats[t].forEach(id=>state.used[t].add(id));let rs='—',bs='—';
 try{rs=agScoreLineup('red',state.seats.red,'',0,false).score;bs=agScoreLineup('blue',state.seats.blue,'',0,false).score}catch(e){}
 const rec={match:state.match,red:[...state.seats.red],blue:[...state.seats.blue],rs,bs,rawRs:rs,rawBs:bs,opponent:state.reOpponent?.name||''};
 if(state.mode==='re'){
   rs=Number(rs);bs=Number(bs);const playerIds=rec[state.playerTeam],hasFav=playerIds.some(id=>byId(id)?.name===state.favorite),cpuSide=other(state.playerTeam),rawPlayer=state.playerTeam==='red'?Number(rec.rawRs):Number(rec.rawBs),rawCpu=cpuSide==='red'?Number(rec.rawRs):Number(rec.rawBs);
   playerIds.forEach(id=>state.rePlayerUsed.add(id));if(hasFav){state.reFavoriteUsed=true;if(state.playerTeam==='red')rs+=5;else bs+=5}
   const keywordResult=applyKeywordEffects(rec.red,rec.blue,rs,bs);rs=keywordResult.red;bs=keywordResult.blue;rec.keywords=keywordResult.events;
   if(hasFav&&Math.abs(rawPlayer-rawCpu)>=40){if(state.playerTeam==='red')rs=rawCpu+1;else bs=rawCpu+1;rec.bondComeback=true}
   if(rs===bs){const luckySide=Math.random()<.5?'red':'blue',raise=Math.random()<.5;if(raise){if(luckySide==='red')rs++;else bs++}else{if(luckySide==='red')bs=Math.max(0,bs-1);else rs=Math.max(0,rs-1)}if(rs===bs){if(luckySide==='red')rs++;else bs++}rec.tieBreak=true}
   rec.rs=rs;rec.bs=bs;const winningSide=rs>bs?'red':'blue',playerWon=winningSide===state.playerTeam;rec.reWinner=playerWon?'player':'cpu';rec.playerTeam=state.playerTeam;rec.favoriteUsed=hasFav;
   rec.trash=trashTalk(rec);const winnerIds=rec[winningSide],winnerTeam=reTeamForSide(winningSide),dialogueTeam=winningSide===state.playerTeam?winnerTeam:(state.reOpponent?.sourceName||winnerTeam);
   if(rec.bondComeback){const fav=byName(state.favorite);rec.reQuote={id:fav.id,name:fav.name,text:'真是受不了你呢，都傷痕累累還要硬拼。',by:`來自 ${fav.name} 無盡的羈絆`,bond:true}}
   else{const charId=winnerIds[Math.floor(Math.random()*winnerIds.length)],char=byId(charId),fallback=rec.trash[Math.floor(Math.random()*rec.trash.length)];rec.reQuote={id:charId,name:char.name,text:reDialogueFor(dialogueTeam,char.name,fallback),by:`${winnerTeam}`}}
 }else{
   rec.trash=trashTalk(rec);
 }
 state.history.push(rec);renderResult(rs,bs)
}
function log(s){state.log.push(s)}
function isCpuTeam(team){return state.playMode==='ai'&&team!==state.playerTeam}
function byName(name){return characters.find(c=>c.name===name)}
function candidateIds(team){return state.pool.filter(id=>!state.banned.has(id)&&!state.seats.red.includes(id)&&!state.seats.blue.includes(id)&&!(['fearless','re'].includes(state.mode)&&state.used[team].has(id)))}
function rankedCandidates(team){const ids=candidateIds(team),picked=state.seats[team].filter(Boolean);return ids.map(id=>{let score=0;try{score=agCandidateStrength(team,picked,id,ids)}catch(e){const c=byId(id);score=c.base*.68+c.tour*.32}return{id,score}}).sort((a,b)=>b.score-a.score).map(x=>x.id)}
function computeLearningMarks(){const out={diamond:new Set(),danger:new Set()};if(!state.learning||!state.phase||state.phase.kind==='rd-remove'||state.phase.kind==='rd-handover')return out;const focus=state.playerTeam||(state.phase.team||state.phase.controller),mine=state.seats[focus].filter(Boolean).length,enemy=other(focus),enemyCount=state.seats[enemy].filter(Boolean).length;if(mine<3)rankedCandidates(focus).slice(0,mine===0?3:mine===1?2:1).forEach(id=>out.diamond.add(id));if(enemyCount>0&&enemyCount<3){const threat=rankedCandidates(enemy)[0];if(threat!=null)out.danger.add(threat)}return out}
function suggestionTeam(){return state.playerTeam||state.phase?.team||state.phase?.controller||'red'}
function refreshSuggestions(){
 const out=state.suggestions;if(!TEST_BUILD||!out?.active)return;const team=suggestionTeam(),lineup=[...state.seats[team]],free=lineup.map((id,index)=>id==null?index:-1).filter(index=>index>=0),candidates=candidateIds(team);out.team=team;out.high=new Set();out.low=new Set();out.highScore=null;out.lowScore=null;if(!free.length||candidates.length<free.length)return;
 const picked=[],usedIds=new Set();function walk(depth){if(depth===free.length){const test=[...lineup];free.forEach((seat,index)=>test[seat]=picked[index]);let score;try{score=Number(agScoreLineup(team,test,'',0,false).score)}catch(e){return}if(!Number.isFinite(score))return;if(out.highScore==null||score>out.highScore){out.highScore=score;out.high=new Set(picked)}if(out.lowScore==null||score<out.lowScore){out.lowScore=score;out.low=new Set(picked)}return}for(const id of candidates){if(usedIds.has(id))continue;usedIds.add(id);picked.push(id);walk(depth+1);picked.pop();usedIds.delete(id)}}walk(0);
}
function toggleSuggestions(){if(!TEST_BUILD||!state.suggestions||state.pendingPick!=null)return;state.suggestions.active=!state.suggestions.active;if(state.suggestions.active)refreshSuggestions();render()}
function bestSeatFor(team,id){const free=state.seats[team].map((x,i)=>x==null?i:-1).filter(i=>i>=0);if(free.length<=1)return free[0];if(state.seats[team].filter(Boolean).length<2)return free[0];let best=free[0],bestScore=-Infinity;for(const seat of free){const test=[...state.seats[team]];test[seat]=id;try{const score=agScoreLineup(team,test,'',0,false).score;if(score>bestScore){bestScore=score;best=seat}}catch(e){}}return best}
function cpuPrivateRemoval(){const team=state.phase.controller,hand=[...state.rdHands[team]].sort((a,b)=>{const ca=byId(a),cb=byId(b);return(ca.base*.68+ca.tour*.32)-(cb.base*.68+cb.tour*.32)});hand.slice(0,3).forEach(id=>state.rdRemoved[team].add(id));state.log=[];if(team==='red'){state.phase={kind:'rd-handover',team:'blue',controller:'blue'};render()}else{const left=characters.map(c=>c.id).filter(id=>!state.rdRemoved.red.has(id)&&!state.rdRemoved.blue.has(id));state.pool=shuffle(left).slice(0,20);beginBan();render();startTimer()}}
function reCpuCandidate(team){
 const available=candidateIds(team),pickNo=state.seats[team].filter(Boolean).length,script=state.reOpponent?.rounds?.[state.match-1]||[],wanted=byName(script[pickNo]);
 if(wanted&&available.includes(wanted.id))return wanted.id;
 const talkers=[...new Set((window.RE_DIALOGUES||[]).filter(x=>x.team&&teamKey(x.team)===teamKey(state.reOpponent?.sourceName||state.reOpponent?.name)).map(x=>byName(x.character)?.id).filter(id=>available.includes(id)))];
 const pool=talkers.length?talkers:available;return pool[Math.floor(Math.random()*pool.length)];
}
function maybeCpuAct(){
 if(state.playMode!=='ai'||!state.phase)return;const snapshot=state.phase;
 if(snapshot.kind==='rd-handover'&&isCpuTeam(snapshot.controller)){setTimeout(()=>{if(state.phase===snapshot)cpuPrivateRemoval()},420);return}
 if(snapshot.kind==='pick'&&snapshot.team===state.playerTeam&&isCpuTeam(other(snapshot.team))&&state.insert[other(snapshot.team)]){setTimeout(()=>{if(state.phase===snapshot&&state.pendingPick==null)useInsert()},520);return}
 if(!isCpuTeam(snapshot.controller))return;
 setTimeout(()=>{if(state.phase!==snapshot)return;
   if(snapshot.kind==='rd-remove'){cpuPrivateRemoval();return}
   if(snapshot.kind==='tavern-ban'){const id=rankedCandidates(other(snapshot.controller))[0]??state.pool.find(availableFor);if(id!=null)choose(id);return}
   if(snapshot.kind==='tavern-pick'){const id=rankedCandidates(snapshot.team)[0]??state.pool.find(availableFor);if(id!=null)choose(id);return}
   if(['ban','bp-ban','insert'].includes(snapshot.kind)){const target=rankedCandidates(other(snapshot.controller))[0]??candidateIds(snapshot.controller)[0];if(target!=null)choose(target);else skipBan();return}
   if(snapshot.kind==='bp-pick'){const id=rankedCandidates(snapshot.team)[0];if(id!=null)choose(id);return}
   if(snapshot.kind==='pick'){const id=state.mode==='re'?reCpuCandidate(snapshot.team):rankedCandidates(snapshot.team)[0];if(id!=null){state.pendingPick=id;placePick(bestSeatFor(snapshot.team,id))}}
 },snapshot.kind==='pick'?1500:650)
}
function cardHTML(id){const c=byId(id),cpuLocked=state.playMode==='ai'&&state.phase&&isCpuTeam(state.phase.controller),pending=state.pendingPick===id,enemyHistoryLocked=['fearless','re'].includes(state.mode)&&['ban','insert'].includes(state.phase?.kind)&&state.used[other(state.phase.controller)].has(id),ownHistoryLocked=['fearless','re'].includes(state.mode)&&['pick','bp-pick'].includes(state.phase?.kind)&&state.used[state.phase.team].has(id),disabled=!availableFor(id)||cpuLocked||state.pendingPick!=null,classes=['vs-card'],pickedNow=state.seats.red.includes(id)||state.seats.blue.includes(id);if(state.banned.has(id))classes.push('banned');if(pickedNow)classes.push('picked');if(pending)classes.push('pending-seat');if(enemyHistoryLocked)classes.push('enemy-history-locked');if(ownHistoryLocked)classes.push('own-history-locked');if(state.marks.diamond.has(id))classes.push('learn-diamond');if(state.marks.danger.has(id))classes.push('learn-danger');if(state.suggestions?.active&&state.suggestions.high.has(id))classes.push('suggest-high');if(state.suggestions?.active&&state.suggestions.low.has(id))classes.push('suggest-low');let usedBadges='';if(['fearless','re'].includes(state.mode)){const redUsed=state.used.red.has(id),blueUsed=state.used.blue.has(id);if(redUsed)classes.push('used-red');if(blueUsed)classes.push('used-blue');if(redUsed&&blueUsed)classes.push('used-both');usedBadges=`<span class="vs-used-badges">${redUsed?'<i class="red">紅方已用</i>':''}${blueUsed?'<i class="blue">藍方已用</i>':''}</span>`}if(state.mode==='rdbp'&&state.phase?.kind==='rd-remove'){classes.push('assigned-'+state.phase.controller);if(state.rdRemoved[state.phase.controller].has(id))classes.push('rd-removed')}return`<button class="${classes.join(' ')}" data-id="${id}" ${disabled?'disabled':''}><span class="vs-avatar-ring"><img src="${c.image}" alt=""></span>${usedBadges}<b>${c.name}</b>${pending?'<small class="vs-place-hint">請在下方座位放置角色</small>':''}</button>`}
function mapSeatHTML(t,i,free){const id=state.seats[t][i],c=id&&byId(id),target=free?.includes(i)&&state.phase?.team===t;return`<div class="vs-map-seat ${t} ${t[0]}${i+1} ${target?'target':''}" data-seat="${i}" data-team="${t}">${t==='red'&&i===0?'<div class="vs-first">FIRST PLAYER</div>':''}${c?`<img src="${c.image}" alt=""><span class="vs-seat-caption"><em>${c.name}</em></span>`:`<span class="vs-seat-empty"><b>${teamName(t)}${i+1}</b><em>${target?'點此放置':'尚未選擇'}</em></span>`}</div>`}
function orderHTML(){const order=state.mode==='tavern'?['紅方1','藍方1','紅方2','藍方2','紅方3','藍方3']:state.mode==='bp'?SEAT_ORDER.map(([t,i])=>`${teamName(t)}${i+1}`):['紅方','藍方','紅方','藍方','藍方','紅方'];return order.map((x,i)=>`<span class="vs-order-node ${x.startsWith('紅')?'red':'blue'}">${x}</span>${i<order.length-1?'<span class="vs-order-arrow">➜</span>':''}`).join('')}
function visiblePool(){return state.phase?.kind==='rd-remove'?[...state.rdHands[state.phase.controller]]:state.pool}
function confirmRdHandover(){state.phase={kind:'rd-remove',team:state.phase.team,controller:state.phase.controller,remaining:3};render();startTimer()}
function render(free){const p=q('versusPanel'),phase=state.phase;if(state.pendingPick!=null&&phase?.kind==='pick'&&!Array.isArray(free))free=state.seats[phase.team].map((id,index)=>id==null?index:-1).filter(index=>index>=0);if(phase?.kind==='rd-handover'){const cpu=isCpuTeam(phase.controller);p.innerHTML=`<div class="vs-draft-shell vs-privacy"><div class="vs-battle-title">RD+BP・私人移除階段</div><div class="vs-privacy-icon">${cpu?'🤖':'🔒'}</div><div class="vs-title">${cpu?'電腦正在處理自己的 26 角':'請將裝置交給'+teamName(phase.controller)}</div><p>${cpu?'電腦的角色名單與移除結果不會顯示。':'上一方的 26 角名單已隱藏。確認周圍沒有對方玩家後，再顯示'+teamName(phase.controller)+'的角色。'}</p>${cpu?'':`<button id="vsReveal" class="vs-btn">我是${teamName(phase.controller)}，顯示我的 26 角</button>`}<button id="vsQuit" class="vs-btn danger">結束本次連戰</button></div>`;if(q('vsReveal'))q('vsReveal').onclick=confirmRdHandover;q('vsQuit').onclick=()=>{if(confirm('確定結束本次連戰並回到首頁？'))quit()};maybeCpuAct();return}const insertTeam=phase?.kind==='pick'?other(phase.team):null,humanController=!isCpuTeam(phase.controller);state.marks=computeLearningMarks();if(state.suggestions?.active)refreshSuggestions();const playerLabel=state.mode==='re'?state.teamName:teamName(state.playerTeam),cpuLabel=state.mode==='re'?state.reOpponent?.name:teamName(other(state.playerTeam));p.innerHTML=`<div class="vs-draft-shell ${['re','fearless','bp','cm01','rdbp','tavern'].includes(state.mode)?'fearless-layout':''} ${state.mode==='re'?'re-draft-shell':''}"><div class="vs-header"><div class="vs-title">${MODES[state.mode].name}${state.mode==='re'?`<small class="re-opponent-name">對戰隊伍：${state.reOpponent?.name||''}</small>`:''}</div><div class="vs-status">第 <b>${state.match}</b> / ${state.total} 場</div></div>${state.playMode==='ai'?`<div class="vs-player-side"><b>${playerLabel}</b> 為 <span class="${state.playerTeam}">${teamName(state.playerTeam)}</span>，<b>${cpuLabel}</b> 為 <span class="${other(state.playerTeam)}">${teamName(other(state.playerTeam))}</span>${isCpuTeam(phase.controller)?'<span class="vs-cpu-thinking">🤖 電腦思考中…</span>':''}</div>`:''}<div class="vs-turn ${phase?.controller||''}">${phaseText()} <span id="vsTimer" class="vs-timer"></span></div><div class="vs-toolbar">${humanController&&['ban','bp-ban','insert'].includes(phase?.kind)?'<button id="vsSkip" class="vs-btn">空過禁角</button>':''}${insertTeam&&state.insert[insertTeam]&&(!state.playerTeam||insertTeam===state.playerTeam)?`<button id="vsInsert" class="vs-btn insert">${teamName(insertTeam)}使用 Insert Ban</button>`:''}<button id="vsQuit" class="vs-btn danger">結束本次連戰</button></div><div class="vs-section-title">${phase?.kind==='rd-remove'?teamName(phase.controller)+'的 26 角（對方不可見）':'可選角色'}</div><div class="vs-pool">${visiblePool().map(cardHTML).join('')}</div><div class="vs-section-title">紅藍選角順序</div><div class="vs-order">${orderHTML()}</div><div class="vs-section-title">模擬座位配置</div><div class="vs-seat-map">${['red','blue'].map(t=>[0,1,2].map(i=>mapSeatHTML(t,i,free)).join('')).join('')}<div class="vs-map-center"><b>${phaseText()}</b><small>${MODES[state.mode].name}<br>紅1為 First Player</small></div></div><div class="vs-log">${state.log.slice(-8).map(x=>`<div>${x}</div>`).join('')}</div></div>`;
 if(state.mode==='re'){
   const fav=byName(state.favorite),oppAvatar=byId(state.reOpponentAvatarId),oppSide=other(state.playerTeam),turn=p.querySelector('.vs-turn'),center=p.querySelector('.vs-map-center'),sub=p.querySelector('.re-opponent-name'),playerSide=p.querySelector('.vs-player-side');
   if(sub)sub.remove();
   if(playerSide)playerSide.innerHTML=`<b class="${state.playerTeam}">${sideLabel(state.playerTeam)}</b>　VS　<b class="${oppSide}">${sideLabel(oppSide)}</b>${isCpuTeam(phase.controller)?'<span class="vs-cpu-thinking">🤖 電腦思考中…</span>':''}`;
   if(turn)turn.innerHTML=`<span class="re-favorite-speaker"><img src="${fav.image}" alt="${esc(fav.name)}"><small>本命</small></span><span class="re-turn-words">${esc(phaseText())}</span><span id="vsTimer" class="vs-timer"></span>`;
   if(center)center.innerHTML=`<span class="re-center-opponent ${oppSide}">${oppAvatar?`<img src="${oppAvatar.image}" alt="${esc(oppAvatar.name)}">`:''}<span><small>本場對戰隊伍</small><b>${esc(state.reOpponent?.name||'')}</b></span></span><small>${esc(phaseText())}<br>紅1為 First Player</small>`;
 }
 const seatMap=p.querySelector('.vs-seat-map');if(seatMap){const arrow=document.createElement('div');arrow.className='vs-direction-arrow';arrow.textContent='➜';arrow.setAttribute('aria-hidden','true');seatMap.appendChild(arrow);if(['re','fearless','bp','cm01','rdbp','tavern'].includes(state.mode)){seatDockCleanup?.();const anchor=document.createElement('div');anchor.className='vs-seat-anchor';seatMap.before(anchor);anchor.appendChild(seatMap);const updateDock=()=>seatMap.classList.toggle('is-docked',anchor.getBoundingClientRect().top>window.innerHeight-seatMap.offsetHeight);window.addEventListener('scroll',updateDock,{passive:true});window.addEventListener('resize',updateDock);seatDockCleanup=()=>{window.removeEventListener('scroll',updateDock);window.removeEventListener('resize',updateDock)};requestAnimationFrame(updateDock)}}
 if(TEST_BUILD&&!String(phase?.kind||'').startsWith('rd-')){const toolbar=p.querySelector('.vs-toolbar');if(toolbar){const button=document.createElement('button');button.id='vsSuggest';button.className=`vs-btn suggest ${state.suggestions?.active?'active':''}`;button.textContent=state.suggestions?.active?'隱藏建議選角':'建議選角';button.disabled=state.pendingPick!=null;button.onclick=toggleSuggestions;toolbar.appendChild(button);if(state.suggestions?.active){const legend=document.createElement('span');legend.className='vs-suggest-legend';legend.innerHTML=`<i class="high"></i>最高分組合 ${state.suggestions.highScore??'—'}　<i class="low"></i>最低分組合 ${state.suggestions.lowScore??'—'}`;toolbar.appendChild(legend)}}}
 const pendingCard=p.querySelector('.vs-card.pending-seat');if(pendingCard)pendingCard.disabled=false;p.querySelectorAll('.vs-card:not(:disabled)').forEach(b=>b.onclick=()=>choose(Number(b.dataset.id)));p.querySelectorAll('.vs-map-seat.target').forEach(e=>e.onclick=()=>placePick(Number(e.dataset.seat)));if(q('vsSkip'))q('vsSkip').onclick=skipBan;if(q('vsInsert'))q('vsInsert').onclick=useInsert;q('vsQuit').onclick=()=>{if(confirm('確定結束本次連戰並回到首頁？'))quit()};renderTimer();maybeCpuAct();}
function avatarRow(ids,large=false,mine=false,label=''){return`<div class="re-lineup-wrap ${mine?'mine':''}"><div class="${large?'vs-result-avatars':'vs-mini-avatars'}">${ids.map(id=>{const c=byId(id);return`<img src="${c.image}" alt="${esc(c.name)}" title="${esc(c.name)}">`}).join('')}</div>${label?`<div class="re-lineup-team">${esc(label)}</div>`:''}</div>`}
function trashTalk(rec){const red=rec.red.map(byId),blue=rec.blue.map(byId),lines=[];try{if(typeof chainRoleTrashTalks==='function')chainRoleTrashTalks({player:red,cpu:blue,specialAudience:[]}).slice(0,2).forEach(x=>lines.push(x))}catch(e){}const winner=rec.rs===rec.bs?'雙方打得難分難解，觀眾決定把鍋留給下一場。':rec.rs>rec.bs?'藍方選完才發現，真正被 Ban 掉的是自己的勝算。':'紅方握有 First Player，卻把勝利先手讓給了藍方。';lines.unshift(winner);return lines.slice(0,3)}
function wireNextButton(finalAction){const button=q('vsNext');if(!button)return;button.type='button';button.onclick=event=>{event.preventDefault();if(button.dataset.busy==='1')return;button.dataset.busy='1';button.disabled=true;button.textContent='載入中…';clearInterval(timerId);state.matchActive=false;if(state.history.length>=state.total)finalAction();else startMatchSafe()}}
function renderResult(rs,bs){
 const last=state.history.at(-1),winner=rs===bs?'平手':rs>bs?'紅方勝':'藍方勝';
 if(state.mode==='re'){
   const mine=last.reWinner==='player'?'勝':'負',quote=last.reQuote,c=byId(quote.id),redMine=last.playerTeam==='red',blueMine=last.playerTeam==='blue';
   q('versusPanel').innerHTML=`<div class="vs-result re-match-result"><div class="vs-battle-title">Re.星杯戰爭・本場結算</div><div class="vs-title">第 ${state.match} 場・${esc(sideLabel('red'))}　VS　${esc(sideLabel('blue'))}</div><div class="vs-lineups"><div class="vs-result-team red"><h3>${esc(sideLabel('red'))}</h3>${avatarRow(last.red,true,redMine,reTeamForSide('red'))}<div class="vs-score red">${rs}</div>${last.favoriteUsed&&redMine?`<small class="re-favorite-score">${last.bondComeback?'本命・無盡羈絆逆轉':'本命羈絆 +5'}</small>`:''}${keywordBadges(last.keywords?.red)}</div><div class="vs-versus">VS</div><div class="vs-result-team blue"><h3>${esc(sideLabel('blue'))}</h3>${avatarRow(last.blue,true,blueMine,reTeamForSide('blue'))}<div class="vs-score blue">${bs}</div>${last.favoriteUsed&&blueMine?`<small class="re-favorite-score">${last.bondComeback?'本命・無盡羈絆逆轉':'本命羈絆 +5'}</small>`:''}${keywordBadges(last.keywords?.blue)}</div></div><h2 class="re-round-verdict ${last.reWinner}">我方${mine}</h2><div class="re-character-dialogue ${quote.bond?'diamond bond':''}"><img src="${c.image}" alt="${esc(c.name)}"><div class="re-dialogue-copy"><b>${esc(c.name)}</b><p>「${esc(quote.text)}」</p><small class="re-quote-by">---by.${esc(quote.by)}</small></div></div><div class="vs-section-title">連戰紀錄</div>${historyHTML()}<div class="vs-toolbar"><button id="vsNext" class="vs-btn">${state.match<state.total?'下一場':'查看最終結算'}</button><button id="vsHome" class="vs-btn">回到首頁</button></div></div>`;
   wireNextButton(renderReFinal);q('vsHome').onclick=quit;return;
 }
 q('versusPanel').innerHTML=`<div class="vs-result"><div class="vs-battle-title">${MODES[state.mode].name}・本場結算</div><div class="vs-title">第 ${state.match} 場完成</div><div class="vs-lineups"><div class="vs-result-team red"><h3>紅方</h3>${avatarRow(last.red,true)}<div class="vs-score red">${rs}</div></div><div class="vs-versus">VS</div><div class="vs-result-team blue"><h3>藍方</h3>${avatarRow(last.blue,true)}<div class="vs-score blue">${bs}</div></div></div><h2>${winner}</h2><div class="vs-trash">${last.trash.map(x=>`<div>「${x}」</div>`).join('')}</div><div class="vs-section-title">連戰紀錄</div>${historyHTML()}<div class="vs-toolbar">${state.history.length<state.total?'<button id="vsNext" class="vs-btn">下一場</button>':'<button id="vsRestart" class="vs-btn">同規則再來一次</button>'}<button id="vsHome" class="vs-btn">回到首頁</button></div></div>`;wireNextButton(startSeries);if(q('vsRestart'))q('vsRestart').onclick=startSeries;q('vsHome').onclick=quit;
}
function reRanking(pattern){
 const rows=window.RE_RANKINGS||[],exact=rows.filter(x=>x.pattern===pattern);if(exact.length)return{...exact[Math.floor(Math.random()*exact.length)],exact:true};
 const wins=[...pattern].reduce((n,x)=>n+(x==='O'?1:x==='D'?.5:0),0),scored=rows.map(x=>{const rw=[...x.pattern].filter(v=>v==='O').length,winGap=Math.abs(rw-wins),roundGap=[...x.pattern].reduce((n,v,i)=>n+(pattern[i]==='D'?.5:v===pattern[i]?0:1),0);return{x,winGap,roundGap}}).sort((a,b)=>a.winGap-b.winGap||a.roundGap-b.roundGap||a.x.rank-b.x.rank),best=scored.filter(x=>x.winGap===scored[0].winGap&&x.roundGap===scored[0].roundGap);return{...best[Math.floor(Math.random()*best.length)].x,exact:false};
}
function secondPlaceTalk(){
 let text='最遙遠的距離，就是獎盃已在你面前，而你捧不起。';try{text=agBossEnding('cpu')?.text||text}catch(e){}
 const rival=state.history.at(-1)?.opponent||'最後一場敵方隊伍',anyRole=byId(shuffle(state.history.flatMap(x=>x[x.playerTeam]))[0])?.name||state.favorite;
 return text.replace(/夢母獅三遷隊|夢母獅隊|母獅隊|母獅/g,rival).replace(/\(我方隊伍名\)/g,state.teamName).replace(/\(任意那一場的角色\)/g,anyRole);
}
const RE_CHAMPION_ENDINGS=["這一刻，所有曾經的遺憾都化成了冠軍獎盃上的光。","五場戰鬥，五次跨越極限，你終於把命運握在自己手中。","星光為你加冕，這一次再也沒有人能奪走你的勝利。","從失敗的盡頭重返賽場，你親手寫下了最耀眼的結局。","你沒有辜負那份穿越時空也想改寫結果的執念。","冠軍不是偶然，而是你與夥伴每一次選擇累積出的答案。","最後一場鐘聲響起時，屬於你的傳說才正要開始。","你走過所有強敵，終於讓自己的隊名刻在星杯之上。","曾經無法跨越的高牆，如今成了你登上頂點的階梯。","這一次你沒有說如果，因為你已經完成了最好的選擇。","所有被質疑的選角，都在冠軍那一刻得到了回答。","五連勝的星芒照亮會場，你成為了這屆唯一的王者。","你與本命並肩走到最後，把不可能變成了冠軍。","曾經失去的榮耀，今天由你親手全部拿了回來。","當獎盃被高高舉起，過往的失敗終於不再令人害怕。","沒有任何劇本比你親手逆轉的命運更加精彩。","你讓三十二支隊伍見證了什麼叫做永不放棄。","最後留下的不是幸運，而是無可取代的判斷與羈絆。","重來一次的機會，被你變成了無可挑剔的勝利。","星杯選擇了你，而你也證明自己配得上這份榮耀。","從第一場到冠軍戰，你的信念從未有一秒動搖。","曾經困住你的陰霾，已被冠軍舞台上的金光徹底驅散。","你把每一場苦戰都化為力量，站上了所有人仰望的位置。","此刻的歡呼屬於隊伍，也屬於那個始終沒有放棄的你。","你與夥伴共同守住最後一分，終於迎來最完美的結局。","這座獎盃證明了，只要再次出發，命運也能被改寫。","沒有遺憾，沒有如果，只有屬於你的五戰全勝。","你在最重要的時刻做對了選擇，冠軍因此有了名字。","穿越回來的你沒有逃避，而是堂堂正正贏回了一切。","星光落在獎盃與夥伴身上，宣告這場戰爭由你勝出。"];
const RE_NON_CHAMPION_ENDINGS=["這次沒有走到頂點，但你已經比記憶中的自己更向前一步。","有些遺憾不會立刻消失，卻會成為下一次出發的力量。","你沒能帶走獎盃，卻留下了一場值得記住的戰鬥。","只差一點並不代表失敗，它證明你已經能看見頂點。","即使結果不如預期，夥伴仍然記得你每一次勇敢的選擇。","這場旅程暫時停在這裡，但你們的故事還沒有結束。","勝敗已經寫下，下一次如何改寫仍掌握在你手中。","你曾經跌倒，也再次站上賽場，這本身就是一種勝利。","沒有奪冠的今天，會成為未來捧杯時最重要的回憶。","別急著否定自己，你已經讓許多不可能動搖了。","有些選擇留下傷痕，也留下下一次不再迷惘的答案。","終點沒有獎盃，但一路並肩的人仍然在你身旁。","星光沒有熄滅，只是等待你下一次抬頭看見。","你輸掉的是這次名次，不是再次挑戰的資格。","即使未能走到最後，你也守住了屬於自己的戰鬥方式。","今天的敗北不是句點，而是一個仍能續寫的逗號。","回頭看吧，你已經跨過了曾經以為無法跨越的地方。","沒有人能保證每次重來都會成功，但你確實改變了某些事。","你與冠軍擦肩而過，卻離下一次勝利更近了。","這一次留下的疑問，下一場就由你親自回答。","隊伍沒有站上頂點，但每一位夥伴都陪你走到了這裡。","輸贏之外，你已經找回了重新相信自己的勇氣。","如果還有不甘，就把它好好保存到下一次對戰。","這場結果也許苦澀，卻比從未挑戰更加珍貴。","你未能改寫全部歷史，至少已經改變了其中一頁。","失去獎盃的瞬間很痛，但那份痛也證明你真心想贏。","別讓名次定義整段旅程，你做過的選擇都有意義。","下一次再入座時，你會比今天更清楚自己為何而戰。","這場戰爭沒有給你完美結局，卻給了你繼續前進的理由。","你和夥伴的羈絆不會因為名次而失去光芒。","即使最後一步沒有踏上去，你也已經走過很長的路。","遺憾仍在，但它已不再是困住你的牢籠。","這次星杯沒有屬於你，下一次可不一定。","你證明了自己能再次面對那場曾經不敢回想的比賽。","結果無法重寫你的努力，也無法抹去夥伴對你的信任。","暫時把不甘收進心裡，等下一次再全部取回來。","敗北讓人沉默，但你的故事不會停在沉默之中。","每一次錯過都在提醒你，真正想守護的是什麼。","你沒有拿到想要的名次，卻沒有再逃離自己的選擇。","下一次鐘聲響起，你仍會是值得信任的隊長。","別害怕再次回到起點，你已經知道前方有哪些風景。","沒有冠軍光環，你與夥伴的努力依然真實。","這場失利終會成為一句笑著說出口的往事。","你距離答案仍有幾步，但方向已經比從前清楚。","今天先讓眼淚落下，明天再帶著它繼續往前。","輸掉一場戰爭，不代表命運已經替你下了結論。","名次停下了，你想要變強的心還在繼續前進。","即使結局仍有遺憾，至少這一次你陪夥伴走到了最後。","把這次選錯的角色記住，也把一起奮戰的人牢牢記住。","當你願意再次按下開始，真正的結局就還沒有到來。"];
function reRankEnding(rank){
 const tier=rank===1?'gold':rank===2?'silver':rank===3?'bronze':rank===32?'wood last':'wood';
 const original=rank===1?'這一次，你終於改寫了命運，站上 Re.星杯戰爭的頂點。':rank===2?secondPlaceTalk():rank===3?'你盡力了，真的。':rank===32?'奪冠是才能，墊底更是憑實力才能獲得的才能！':'即使轉生也好想再奪冠一場...真的好想啊....';
 const pool=rank===1?[original,...RE_CHAMPION_ENDINGS]:[original,...RE_NON_CHAMPION_ENDINGS];
 return{tier,text:pool[Math.floor(Math.random()*pool.length)]};
}
function reChibiUrl(name){return`assets/chibi/${encodeURIComponent(name)}.webp`}
const RE_FAVORITE_FINISH_LINES=[
 '不管重來多少次，我都會再次站到你身邊。','你選擇了我，那我就陪你戰到最後一刻。','這一路的勝敗不重要，重要的是我們一起走到了這裡。','只要你還願意呼喚我，我就永遠不會缺席。','別怕，下一場戰鬥，我依然會站在你前面。',
 '你的信任我收到了，接下來就交給我吧。','即使全世界都不看好我們，我也會陪你贏到最後。','能與你並肩奮戰，就是我最想守護的榮耀。','我們留下的傷痕，終有一天會成為勝利的勳章。','你沒有放棄我，我又怎麼可能先放棄你？',
 '這不是最後一戰，而是我們下一段傳說的開始。','無論前方還有多少強敵，我都會陪你一起突破。','只要我們還站在一起，就沒有真正的敗北。','你的每一次選擇，都讓我有了繼續戰鬥的理由。','如果命運不肯讓路，那我們就一起把它擊碎。',
 '我會回應你的期待，直到最後一道光熄滅為止。','勝利也好，失敗也罷，我從未後悔與你並肩。','就算遍體鱗傷，我也要陪你走到故事的終點。','你將背後交給了我，我便不會讓任何人越過這裡。','因為是你選中了我，所以我才能戰鬥到現在。',
 '我們不是偶然相遇，而是注定要一起完成這場戰爭。','記住此刻吧，這是只屬於我們的光榮。','只要你的心還沒有認輸，我的劍就不會停下。','你負責相信我，我負責替你開闢通往勝利的道路。','即使再回到過去一百次，我仍然希望被你選中。',
 '這一次，我們沒有留下任何遺憾。','讓那些懷疑我們的人，好好見證這份羈絆吧。','別把所有重擔都扛在自己身上，我一直都在。','你的願望就是我的方向，你的戰場就是我的歸宿。','這份並肩作戰的回憶，我永遠都不會忘記。',
 '不需要奇蹟，因為你我站在一起就是答案。','如果你累了，就讓我替你撐住下一個回合。','我們走過的每一步，都比獎盃更加珍貴。','你曾經相信我一次，我便願意守護你直到永遠。','即使前方只剩絕境，我也會陪你打出一條生路。',
 '別回頭，我會守住我們一起走過的道路。','能成為你的本命，是我比奪冠更驕傲的事。','這場戰爭證明了，我們的相遇絕非偶然。','只要你再次伸出手，我就會毫不猶豫地回到你身邊。','我們曾一起跌倒，所以更懂得如何一起站起來。',
 '今天由我守護你的勝利，明天也不會例外。','不論最後站上哪個位置，你永遠都是我認可的夥伴。','真正讓我變得強大的，從來不是分數，而是你的信任。','你帶我走過了五場戰鬥，接下來換我陪你走得更遠。','別說結束，我們的羈絆才正要開始閃耀。',
 '就算星光全數墜落，我也會成為照亮你的那一道光。','我們已經改寫過一次命運，也一定能再改寫下一次。','謝謝你直到最後，都沒有鬆開牽著我的手。','下一次鐘聲響起時，我還要與你坐在同一個位置。','無論經過多少輪迴，我都會記得你曾選擇過我。'
];
function reFavoriteEndingHTML(victory){
 const fav=byName(state.favorite);if(state.reFavoriteUsed){if(!state.reFavoriteEndingText)state.reFavoriteEndingText=RE_FAVORITE_FINISH_LINES[Math.floor(Math.random()*RE_FAVORITE_FINISH_LINES.length)];return`<div class="re-congrats diamond"><img src="${fav.image}" alt="${esc(fav.name)}"><div><b>${victory?`恭喜 ${esc(state.teamName)} 贏得 Re.星杯戰爭！`:`恭喜 ${esc(state.teamName)} 獲得第 ${state.reFinal?.rank||''} 名！`}</b><p>「${esc(state.reFavoriteEndingText)}」</p></div></div>`}
 const neglect=agFavoriteNeglectEgg(state.favorite,false,state.reFavoriteSeen);state.reFavoriteEndingText=neglect?.text||'原來回到過去，你還是沒有選我。';return`<div class="re-neglect"><img src="${fav.image}" alt="${esc(fav.name)}"><div><b>${esc(fav.name)}</b><p>「${esc(state.reFavoriteEndingText)}」</p></div></div>`;
}
function renderReFinal(){
 clearInterval(timerId);const wins=state.history.filter(x=>x.reWinner==='player').length,losses=state.history.length-wins,pattern=state.history.map(x=>x.reWinner==='player'?'O':'X').join(''),ranking=reRanking(pattern),victory=wins===5,ending=reRankEnding(victory?1:ranking.rank);
 const spark=Array.from({length:22},(_,i)=>`<i style="--i:${i};--x:${(i*47)%100}%;--d:${(i%7)*.16}s"></i>`).join('');
 const rank=victory?1:ranking.rank,rankNote=victory?'五戰全勝，奪得 32 隊賽事冠軍。':`${ranking.exact?'與參考賽程隊伍 '+ranking.team+' 完全相同。':'依勝場數與逐輪結果，比照最接近的隊伍 '+ranking.team+'。'}`;state.reFinal={wins,losses,pattern,rank,ending,rankNote};
 const last=state.history.at(-1),lastMine=last?.[last.playerTeam]||[],championRole=state.reFinal.championChibi||(state.reFavoriteUsed?state.favorite:byId(lastMine[Math.floor(Math.random()*lastMine.length)])?.name),usedBard=state.history.some(match=>(match[match.playerTeam]||[]).some(id=>byId(id)?.name==='吟游詩人')),wheelchair=rank>3&&usedBard;if(victory)state.reFinal.championChibi=championRole;state.reFinal.wheelchairBard=wheelchair;
 q('versusPanel').innerHTML=`<div class="re-final ${victory?'victory':'defeat'}">${victory?`<div class="re-confetti">${spark}</div><div class="re-trophy-scene"><img class="re-chibi re-chibi-trophy" src="${reChibiUrl(championRole)}" alt="${esc(championRole)} Q版"><img class="re-cup" src="assets/anniversary_cup.png" alt="冠軍獎盃"></div>`:''}<div class="re-final-title">Re.星杯戰爭・${victory?'冠軍':'最終名次'}</div><div class="re-rank-scene">${wheelchair?`<img class="re-chibi re-wheelchair" src="${reChibiUrl('輪椅吟遊詩人')}" alt="輪椅吟遊詩人">`:''}<div class="re-rank-number tier-${ending.tier.split(' ')[0]}">第 ${rank} 名 <small>/ 32 隊</small></div></div><div class="re-rank-ending ${ending.tier}">「${esc(ending.text)}」</div>${reFavoriteEndingHTML(victory)}<div class="re-record">${wins} 勝・${losses} 負</div>${historyHTML()}<div class="vs-toolbar"><button id="reSaveImage" class="vs-btn re-save-btn">儲存圖片</button><button id="vsRestart" class="vs-btn">重新挑戰</button><button id="vsHome" class="vs-btn">回到首頁</button></div></div>`;
 q('reSaveImage').onclick=saveReResultImage;q('vsRestart').onclick=startSeries;q('vsHome').onclick=quit;
}
function historyHTML(){const re=state.mode==='re';return`<table class="vs-history ${re?'re-history':''}"><thead><tr><th>場次</th><th>紅方</th><th>紅分</th><th>藍方</th><th>藍分</th><th>${re?'我方結果':'結果'}</th></tr></thead><tbody>${state.history.map(h=>{const redMine=re&&h.playerTeam==='red',blueMine=re&&h.playerTeam==='blue',mineKeywords=re?keywordBadges(h.keywords?.[h.playerTeam],false):'';return`<tr><td>${h.match}</td><td>${avatarRow(h.red,false,redMine,re?redMine?state.teamName:h.opponent:'')}</td><td class="vs-score red" style="font-size:20px">${h.rs}</td><td>${avatarRow(h.blue,false,blueMine,re?blueMine?state.teamName:h.opponent:'')}</td><td class="vs-score blue" style="font-size:20px">${h.bs}</td><td class="${re?(h.reWinner==='player'?'re-win':'re-loss'):''}">${re?(h.reWinner==='player'?'勝':'負'):(h.rs===h.bs?'平手':h.rs>h.bs?'紅方勝':'藍方勝')}${mineKeywords}</td></tr>`}).join('')}</tbody></table>`}
function reLoadImage(src){if(typeof loadCanvasSafeImage==='function')return loadCanvasSafeImage(src).catch(()=>null);return new Promise(resolve=>{const image=new Image();image.onload=()=>resolve(image);image.onerror=()=>resolve(null);image.src=src})}
function reCanvasCircle(ctx,image,x,y,r,border){ctx.save();ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.clip();if(image)ctx.drawImage(image,x-r,y-r,r*2,r*2);ctx.restore();ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.lineWidth=5;ctx.strokeStyle=border;ctx.stroke()}
function reCanvasWrap(ctx,text,x,y,maxWidth,lineHeight,maxLines=3){let line='',lines=[];for(const ch of String(text)){const next=line+ch;if(ctx.measureText(next).width>maxWidth&&line){lines.push(line);line=ch}else line=next}if(line)lines.push(line);lines.slice(0,maxLines).forEach((value,index)=>ctx.fillText(value,x,y+index*lineHeight));return y+Math.min(lines.length,maxLines)*lineHeight}
function reCanvasRoundedBox(ctx,x,y,w,h,r,fill,stroke,lineWidth=3){ctx.save();ctx.beginPath();ctx.moveTo(x+r,y);ctx.lineTo(x+w-r,y);ctx.quadraticCurveTo(x+w,y,x+w,y+r);ctx.lineTo(x+w,y+h-r);ctx.quadraticCurveTo(x+w,y+h,x+w-r,y+h);ctx.lineTo(x+r,y+h);ctx.quadraticCurveTo(x,y+h,x,y+h-r);ctx.lineTo(x,y+r);ctx.quadraticCurveTo(x,y,x+r,y);ctx.closePath();ctx.fillStyle=fill;ctx.fill();ctx.strokeStyle=stroke;ctx.lineWidth=lineWidth;ctx.stroke();ctx.restore()}
function reCanvasContain(ctx,image,x,y,w,h){if(!image)return;const scale=Math.min(w/image.width,h/image.height),dw=image.width*scale,dh=image.height*scale;ctx.drawImage(image,x+(w-dw)/2,y+(h-dh)/2,dw,dh)}
async function saveReResultImage(){
 const button=q('reSaveImage');if(button){button.disabled=true;button.textContent='圖片產生中…'}
 try{
   const final=state.reFinal,fav=byName(state.favorite),victory=final.rank===1,sectionY=victory?350:90,historyY=sectionY+545,rowHeight=140,W=1400,H=historyY+state.history.length*rowHeight+105,canvas=document.createElement('canvas');canvas.width=W;canvas.height=H;const ctx=canvas.getContext('2d'),font='"Noto Sans TC","PingFang TC","Microsoft JhengHei",sans-serif';
   const allIds=[...new Set(state.history.flatMap(h=>[...h.red,...h.blue]))],images=new Map(await Promise.all(allIds.map(async id=>[id,await reLoadImage(byId(id).image)]))),favImage=await reLoadImage(fav.image),championName=final.championChibi||state.favorite,chibiSource=(typeof chibiPath==='function'?chibiPath(championName):reChibiUrl(championName)),cupSource=(typeof RE_CUP_IMAGE!=='undefined'?RE_CUP_IMAGE:'assets/anniversary_cup.png'),wheelchairSource=(typeof chibiPath==='function'?chibiPath('輪椅吟遊詩人'):reChibiUrl('輪椅吟遊詩人'));
   const [cupImage,chibiImage,wheelchairImage]=await Promise.all([victory?reLoadImage(cupSource):null,victory?reLoadImage(chibiSource):null,(!victory&&final.wheelchairBard)?reLoadImage(wheelchairSource):null]);
   const gradient=ctx.createLinearGradient(0,0,0,H);gradient.addColorStop(0,'#162643');gradient.addColorStop(1,'#070d1c');ctx.fillStyle=gradient;ctx.fillRect(0,0,W,H);ctx.strokeStyle='#d9b85f';ctx.lineWidth=5;ctx.strokeRect(24,24,W-48,H-48);
   if(victory){ctx.save();ctx.shadowColor='rgba(255,206,69,.55)';ctx.shadowBlur=25;reCanvasContain(ctx,chibiImage,350,42,310,265);reCanvasContain(ctx,cupImage,665,25,310,290);ctx.restore()}else if(final.wheelchairBard){reCanvasContain(ctx,wheelchairImage,165,18,230,210)}
   ctx.textAlign='center';ctx.fillStyle='#f5cf72';ctx.font=`1000 42px ${font}`;ctx.fillText(`Re.星杯戰爭・${victory?'冠軍':'最終名次'}`,W/2,sectionY);
   ctx.fillStyle=final.rank===2?'#d9e2ef':final.rank===3?'#d69061':final.rank===1?'#ffe18a':'#d4b185';ctx.font=`1000 74px ${font}`;ctx.fillText(`第 ${final.rank} 名`,W/2,sectionY+82);
   ctx.fillStyle='#dfe8fb';ctx.font=`800 27px ${font}`;ctx.fillText(`${state.teamName}　${final.wins} 勝・${final.losses} 負`,W/2,sectionY+130);
   const endingY=sectionY+165,endingStyle=final.rank===1?['rgba(90,64,23,.88)','#e8c55f','#fff0b2']:final.rank===2?['rgba(56,70,90,.92)','#c9d1df','#f2f6ff']:final.rank===3?['rgba(84,49,31,.92)','#b96f46','#f4d2bd']:['rgba(75,44,22,.94)','#9a6537','#ffe2ba'];reCanvasRoundedBox(ctx,105,endingY,W-210,122,20,endingStyle[0],endingStyle[1],5);ctx.fillStyle=endingStyle[2];ctx.font=`750 25px ${font}`;ctx.textAlign='center';reCanvasWrap(ctx,`「${final.ending.text}」`,W/2,endingY+45,W-300,35,3);
   const favY=sectionY+307;reCanvasRoundedBox(ctx,105,favY,W-210,145,22,state.reFavoriteUsed?'rgba(35,47,78,.96)':'rgba(42,28,57,.94)',state.reFavoriteUsed?'#aef3ff':'#8c6f9f',4);reCanvasCircle(ctx,favImage,190,favY+72,48,state.reFavoriteUsed?'#baf2ff':'#c6a1dc');ctx.textAlign='left';ctx.fillStyle='#ffe39a';ctx.font=`900 24px ${font}`;ctx.fillText(state.favorite,260,favY+45);ctx.fillStyle='#f1f5ff';ctx.font=`600 20px ${font}`;reCanvasWrap(ctx,`「${state.reFavoriteEndingText}」`,260,favY+79,990,29,3);
   ctx.textAlign='center';ctx.fillStyle='#fff0bb';ctx.font=`900 27px ${font}`;ctx.fillText('連戰紀錄',W/2,sectionY+505);
   let y=historyY;ctx.font=`800 21px ${font}`;for(const h of state.history){ctx.fillStyle='rgba(22,35,62,.96)';ctx.fillRect(65,y-38,W-130,124);ctx.strokeStyle='#40577f';ctx.lineWidth=2;ctx.strokeRect(65,y-38,W-130,124);ctx.fillStyle='#f4d06f';ctx.textAlign='left';ctx.fillText(`第 ${h.match} 場`,88,y+10);const redMine=h.playerTeam==='red',blueMine=h.playerTeam==='blue',redName=redMine?state.teamName:h.opponent,blueName=blueMine?state.teamName:h.opponent;ctx.textAlign='center';ctx.fillStyle='#ff9aaa';ctx.fillText(`紅方・${redName}`,385,y-9);ctx.fillStyle='#91c4ff';ctx.fillText(`藍方・${blueName}`,1015,y-9);for(let i=0;i<3;i++){reCanvasCircle(ctx,images.get(h.red[i]),300+i*76,y+36,29,redMine?'#f3ce63':'#a65465');reCanvasCircle(ctx,images.get(h.blue[i]),930+i*76,y+36,29,blueMine?'#f3ce63':'#4f83bd')}ctx.fillStyle='#ff91a3';ctx.font=`900 29px ${font}`;ctx.fillText(String(h.rs),640,y+19);ctx.fillStyle='#7db7ff';ctx.fillText(String(h.bs),760,y+19);ctx.fillStyle=h.reWinner==='player'?'#ffe18a':'#ff91a3';ctx.font=`1000 32px ${font}`;ctx.fillText(h.reWinner==='player'?'勝':'負',700,y+60);const keywords=(h.keywords?.[h.playerTeam]||[]).map(event=>event.name).join('・');if(keywords){ctx.fillStyle='#b7c8e9';ctx.font=`700 14px ${font}`;ctx.fillText(keywords,700,y+81)}ctx.font=`800 21px ${font}`;y+=rowHeight}
   ctx.textAlign='center';ctx.fillStyle='#8291b2';ctx.font=`500 16px ${font}`;ctx.fillText('Re.星杯戰爭・五連戰紀錄',W/2,H-35);
   const blob=await new Promise((resolve,reject)=>canvas.toBlob(value=>value?resolve(value):reject(new Error('瀏覽器無法建立 PNG 圖片')),'image/png'));
   const overlay=document.getElementById('iosSaveOverlay'),preview=document.getElementById('iosSaveImage'),openLink=document.getElementById('iosSaveOpen'),downloadLink=document.getElementById('iosSaveDownload'),closeButton=document.getElementById('iosSaveClose');if(!overlay||!preview||!openLink||!downloadLink||!closeButton)throw new Error('找不到圖片預覽視窗');
   if(rePreviewObjectUrl)URL.revokeObjectURL(rePreviewObjectUrl);rePreviewObjectUrl=URL.createObjectURL(blob);preview.src=rePreviewObjectUrl;preview.alt=`${state.teamName} 的 Re.星杯戰爭結果圖片`;openLink.href=rePreviewObjectUrl;downloadLink.href=rePreviewObjectUrl;downloadLink.download=`Re星杯戰爭_${state.teamName}_第${state.reFinal.rank}名.png`;closeButton.onclick=()=>{overlay.classList.remove('show');preview.removeAttribute('src');setTimeout(()=>{if(rePreviewObjectUrl){URL.revokeObjectURL(rePreviewObjectUrl);rePreviewObjectUrl=''}},300)};overlay.classList.add('show');
 }catch(error){alert('圖片產生失敗：'+error.message)}finally{if(button){button.disabled=false;button.textContent='儲存圖片'}}
}
function quit(){clearInterval(timerId);clearTimeout(storyTimer);q('versusPanel').classList.add('hidden');q('profilePanel').classList.remove('hidden');window.scrollTo({top:0,behavior:'smooth'})}
selectMode('bp');
})();
