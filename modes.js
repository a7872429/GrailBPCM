...
let state=null,timerId=null,storyTimer=null,learningEnabled=false;
...
function renderResult(rs,bs){
 ...
 if(state.mode==='re'){
   ...
   q('vsNext').onclick=state.history.length<state.total?startMatch:renderReFinal;q('vsHome').onclick=quit;return;
 }
...
function renderReFinal(){
 ...
 q('versusPanel').innerHTML=`<div class="re-final ${victory?'victory':'defeat'}">${victory?`<div class="re-confetti">${spark}</div><img class="re-cup" src="assets/anniversary_cup.png" alt="冠軍獎盃">`:''}<div class="re-final-title">Re.星杯戰爭・${victory?'冠軍':'最終名次'}</div><div class="re-rank-number tier-${ending.tier.split(' ')[0]}">第 ${rank} 名 <small>/ 32 隊</small></div><div class="re-rank-ending ${ending.tier}">「${esc(ending.text)}」</div>${reFavoriteEndingHTML(victory)}<div class="re-record">${wins} 勝・${losses} 負</div>${historyHTML()}<div class="vs-toolbar"><button id="reSaveImage" class="vs-btn re-save-btn">儲存圖片</button><button id="vsRestart" class="vs-btn">重新挑戰</button><button id="vsHome" class="vs-btn">回到首頁</button></div></div>`;
