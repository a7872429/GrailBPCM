...
async function saveReResultImage(){
 ...
 try{
   const final=state.reFinal,fav=byName(state.favorite),victory=final.rank===1,sectionY=victory?340:50,historyY=sectionY+545,rowHeight=140,W=1400,H=historyY+state.history.length*rowHeight+105,canvas=document.createElement('canvas');canvas.width=W;canvas.height=H;const ctx=canvas.getContext('2d'),font='"Noto Sans TC","PingFang TC","Microsoft JhengHei",sans-serif';
   ...
   ctx.textAlign='center';ctx.fillStyle='#f5cf72';ctx.font=`1000 48px ${font}`;ctx.fillText(`Re.星杯戰爭・${victory?'冠軍':'最終名次'}`,W/2,sectionY);
